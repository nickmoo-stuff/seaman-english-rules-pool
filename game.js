...
function restartCurrentGame(breaker=1){if(currentScenario)return startScenario(currentScenario);newFrame(breaker);}
...
document.getElementById('unlockScenarios').onclick=()=>{devScenariosUnlocked=true;nightmareScenario.disabled=false;cannonScenario.disabled=false;nightmareScenario.querySelector('small').textContent='Captain Blackball • 3 reds vs 6-yellow blockade';cannonScenario.querySelector('small').textContent='Captain Blackball • oversized 28-ball rack';msg.textContent='DEV: Test scenarios unlocked for this session.';};
  ...
  if(!state.breakShot){
    ...
    if(pots.length){
      ...
      if(currentPirateLevel>=4&&eligible.length){
        ...
        if(safety&&attackUgly&&safetyClearlyBetter){
          safety.rejectedAttack={reason:`marginal attack (cut ${bestAttack.cutDeg.toFixed(1)}°, route ${bestAttack.score.toFixed(0)})`,bestScore:bestAttack.score,target:bestAttack.target};
          safety.personalityInfluence=`SAFETY preferred (threshold ${safetyThreshold.toFixed(2)})`;
          return safety;
        }
      }
...
function showModeMenu(){currentScenario=null;playersModal.hidden=false;modeMenu.hidden=false;playersForm.hidden=true;piratePlaceholder.hidden=true;if(testScenarioMenu)testScenarioMenu.hidden=true;if(aiVsAiMenu)aiVsAiMenu.hidden=true;refreshPirateButtons();}
...
testMode.onclick=()=>{audioReady();modeMenu.hidden=true;playersForm.hidden=true;piratePlaceholder.hidden=true;testScenarioMenu.hidden=false;nightmareScenario.disabled=!devScenariosUnlocked;cannonScenario.disabled=!devScenariosUnlocked;testPlayerName?.focus();};
testBack.onclick=showModeMenu;nightmareScenario.onclick=()=>{if(devScenariosUnlocked)startScenario('nightmare');};cannonScenario.onclick=()=>{if(devScenariosUnlocked)startScenario('cannon');};

...
function addHoldNudge(id,delta){const el=document.getElementById(id);let delay=null,repeat=null;const stop=()=>{clearTimeout(delay);clearInterval(repeat);delay=repeat=null};el.addEventListener('pointerdown',e=>{e.preventDefault();nudgeAngle(delta);delay=setTimeout(()=>repeat=setInterval(()=>nudgeAngle(delta),45),350)});['pointerup','pointercancel','pointerleave'].forEach(ev=>el.addEventListener(ev,stop));el.addEventListener('contextmenu',e=>e.preventDefault())}addHoldNudge('angleMinus',-.1);addHoldNudge('anglePlus',.1);
